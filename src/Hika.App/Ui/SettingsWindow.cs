...
public sealed class SettingsWindow : Form
{
    ...
    private Control BuildFooter()
    {
        ...
        // Не «Что я слышу»: так теперь называется раздел в самом окне,
        // а эта кнопка открывает отдельное окно консоли — вещь другого рода.
        var listen = new FlatButton("Живая проверка") { Width = 150 };
    ...
    private Control BuildPersonaPage()
    {
        ...
        return Stack(
            ...
            new SettingRow("Свои варианты произношения",
                "Через запятую. Впишите сюда то, как имя реально расслышала модель — это надёжнее любой " +
                "подстройки порогов. Посмотреть, как именно она его слышит, можно в разделе «Что происходит».",
                _extraVariants),
