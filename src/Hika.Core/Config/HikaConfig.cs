...
public sealed class OverlayConfig
{
    ...
    /// <summary>
    /// Светиться, ещё не зная, к нам ли обращаются.
    ///
    /// Выключено, и это было ошибкой в обратную сторону. Задумывалось как
    /// вежливость: человек видит, что его услышали, ещё до того, как имя
    /// распозналось. На деле получилось, что кайма вспыхивает от любого звука
    /// в комнате — от разговора рядом, от видео, от кашля, — и вместо
    /// «я тебя слушаю» читается как «программа мигает сама по себе».
    ///
    /// Свечение обязано означать ровно одно: прозвучало имя. Иначе оно
    /// перестаёт что-либо значить.
    /// </summary>
    public bool ShowBeforeWakeWord { get; set; } = false;

    /// <summary>
    /// Яркость этого едва заметного свечения. Действует, только если включено
    /// <see cref="ShowBeforeWakeWord"/>.
    /// </summary>
