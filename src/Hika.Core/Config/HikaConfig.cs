...
public sealed class HikaConfig
{
    ...
    ///
    /// Нужна ради одной неприятной особенности: файл создаётся при первом
    /// запуске со всеми значениями по умолчанию, выписанными явно. Дальше
    /// изменить значение по умолчанию в коде бессмысленно — у человека
    /// в файле лежит старое, и он останется со старым навсегда, не понимая,
    /// почему у него медленнее, чем обещано.
    ///
    /// Поэтому при подъёме версии значения, которых человек не касался,
    /// подтягиваются к новым. Изменённые вручную не трогаются: сравнение
    /// идёт ровно со старым умолчанием.
    /// </summary>
...
public sealed class AudioConfig
{
    ...
    /// распознавание даже не начнётся, сколько бы его ни ускоряли.
    ///
    /// Было полсекунды — вышло слишком осторожно. Ошибиться здесь дёшево
    /// в обе стороны: слишком мало — фраза разрежется на паузе, и остаток
    /// придёт следующей репликой, которую всё равно примут как продолжение;
    /// слишком много — человек ждёт каждый раз. Четыреста миллисекунд
    /// заметно живее и всё ещё длиннее любой паузы между словами.
    /// </summary>
    public int SilenceMs { get; set; } = 400;
...
public sealed class SpeechConfig
{
    ...
    /// <summary>Потоков распознавания. 0 — все ядра, кроме пары оставленных системе (но не больше восьми).</summary>
    ...
    /// <summary>
    /// Сколько речи накопить перед первой проверкой имени, мс.
    ///
    /// Ровно та задержка, через которую загорается кайма. Меньше трёхсот
    /// смысла не имеет: имя ещё физически не произнесено. А вот больше —
    /// это уже чистое ожидание, которое человек видит глазами.
    /// </summary>
    public int ProbeAfterMs { get; set; } = 350;

    /// <summary>
    /// Как часто повторять проверку, пока имя не найдено, мс.
    ///
    /// Раньше проверок было ровно две, и между ними зияла секунда, в которую
    /// имя, произнесённое не с первого слога, не находилось вовсе. Скользящая
    /// проверка стоит десятки миллисекунд и закрывает этот разрыв целиком.
    /// </summary>
    public int ProbeIntervalMs { get; set; } = 200;

    /// <summary>
    /// Сколько последних миллисекунд речи отдавать проверке.
    ///
    /// Постоянная ширина — то, что делает скользящую проверку возможной:
    /// иначе стоимость росла бы вместе с длиной фразы, и к третьей проверке
    /// она стала бы дороже самой команды.
    /// </summary>
    public int ProbeWindowMs { get; set; } = 1400;

    /// <summary>
    /// Подгонять окно кодировщика под длину фразы.
    ///
    /// Самый большой запас по отзывчивости во всей программе: whisper всегда
    /// считает окно в тридцать секунд, дополняя короткую команду тишиной,
    /// и почти вся работа уходит на эту тишину. Подробности —
    /// в <see cref="Hika.Stt.WhisperTuning"/>.
    ///
    /// Выключать стоит, только если появились странности в распознавании
    /// длинных фраз.
    /// </summary>
    public bool AdaptiveContext { get; set; } = true;

    /// <summary>
    /// Быстрое декодирование: без переспрашивания и без перебора вариантов.
    ///
    /// Не сойдясь с порогами уверенности, whisper по умолчанию перезапускает
    /// расшифровку с другой температурой — и так до пяти раз. Для расшифровки
    /// лекции это правильно, для команды из трёх слов означает пятикратное
...
public sealed class OverlayConfig
{
    ...
    /// <summary>
    /// Толщина каймы в долях меньшей стороны экрана. 0.07 ≈ 75 px на 1080p.
    ///
    /// Свет занимает не всю эту полосу, а её внешнюю треть — остальное уходит
    /// в мягкое затухание. Поэтому число выглядит больше, чем видно на экране.
    /// </summary>
    public double Thickness { get; set; } = 0.07;
...
public sealed class BehaviorConfig
{
    ...
    /// <summary>
    /// Чем включать музыку по команде «включи музыку». Пусто — выбрать самой
    /// из установленного: Apple Music, Spotify, Яндекс Музыка и далее по списку.
    /// </summary>
    public string MusicApp { get; set; } = "";

    /// <summary>
    /// Сколько секунд после выполненной команды слушать следующую без имени.
    ///
    /// Разговор с ассистентом редко состоит из одной фразы: за «поставь паузу»
    /// почти всегда идёт «а теперь громче». Требовать имя перед каждой —
    /// значит заставлять человека обращаться к программе вместо того,
    /// чтобы ей пользоваться.
    ///
    /// Свечение при этом гаснет: гореть всё это время оно не должно, иначе
    /// «слушаю» превращается в постоянный фон. Ноль — выключить.
    /// </summary>
    public int FollowUpSeconds { get; set; } = 8;
    ...
    /// <summary>
    /// Клавиша «слушай». Пусто — не назначать.
    ///
    /// Одно нажатие — и команду можно говорить без имени; выполнила и снова
    /// спит. Нужно чаще, чем кажется: имя приходится произносить вслух,
    /// а вслух не везде удобно — рядом спят, идёт запись, в наушниках созвон.
    /// Нажатие решает это молча.
    ///
    /// Второе нажатие, пока она слушает, — отбой: нажали случайно, и ждать
    /// зря не надо.
    /// </summary>
    public string ListenHotkey { get; set; } = "Ctrl+Alt+Space";
...
public sealed class VoiceConfig
{
    ...
    /// <summary>
    /// Говорить только нейроголосом.
    ///
    /// Старые голоса Windows склеены из кусочков записи и звучат как компьютер
    /// из двухтысячных. Взять такой, когда нейроголоса нет, — решение, которое
    /// выглядит вежливым («хоть что-то сказала») и на слух невыносимо.
    /// Молчание честнее: оно хотя бы не раздражает и отправляет ставить
    /// нормальный голос.
    /// </summary>
    public bool NeuralOnly { get; set; } = true;
