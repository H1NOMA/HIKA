...
public sealed record Hotkey(uint Modifiers, uint Key, string Text)
{
    ...
    private static string Display(string part) => part switch
    {
        ...
        // Записываем ровно так, как потом разберём: показанное человеку
        // сочетание и есть то, что ляжет в настройки, и оно обязано
        // прочитаться обратно без потерь.
        "плюс" or "равно" => "Плюс",
        "минус" or "дефис" => "Минус",
        "точказапятая" => "ТочкаЗапятая",
        "скобкалевая" => "СкобкаЛевая",
        "скобкаправая" => "СкобкаПравая",
        "обратныйслэш" => "ОбратныйСлэш",
    ...
    private static uint KeyCode(string name)
    {
        ...
        return name switch
        {
            ...
            // Знаки в основном ряду. Это не то же самое, что на цифровой
            // клавиатуре: у них разные коды, и назначенное «Plus» человек
            // потом безуспешно нажимал в верхнем ряду, не понимая, почему
            // записанное в настройках сочетание не срабатывает.
            "плюс" or "равно" => 0xBB,
            "минус" or "дефис" => 0xBD,
            "запятая" => 0xBC,
            "точка" => 0xBE,
