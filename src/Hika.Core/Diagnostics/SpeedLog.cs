...
public readonly record struct SpeedSample
{
    ...
    /// <summary>
    /// Сколько человек ждал текста — от конца фразы до разобранной строки.
    ///
    /// Именно ждал, а не «модель считала». Разница появляется, когда модель
    /// ещё догружается или занята предыдущей фразой: счёт при этом занимает
    /// свои триста миллисекунд, а человек ждёт четыре секунды. Раньше эта
    /// разница попадала в «действие», и программа советовала чинить запуск
    /// программ — единственное, чего в тот момент не происходило вовсе.
    /// </summary>
    public int RecognitionMs { get; init; }

    /// <summary>
    /// Из них — собственно счёт модели. Всё остальное это очередь, загрузка
    /// и подготовка. Нужно для отношения к длине речи: оно про то, успевает
    /// ли модель за речью, а очередь к её быстроте отношения не имеет.
    /// </summary>
    ...
    /// </summary>
    public double RealTime => AudioMs <= 0
        ? 0
        : (DecodeMs > 0 ? DecodeMs : RecognitionMs) / (double)AudioMs;
}
